...
def linkerseqCo(path, dbtype = 1):
    if dbtype == 0:
        ...
        for j in range(len(allstring)):      
            s = allstring[j]
            if s.startswith("0") or s.startswith("x") or s.startswith("1"):
                linkerstr.append(allstring[j])
                protein.append(allstring[j-1])
    return protein, linkerstr     
...
def getlinkerset(path):
    ...
    return linkerset 
c = lineseq('./dataset/TRAINING.fasta') 
a = linkerseq('./dataset/TRAINING.fasta') 
#b = getlinkerset('./DM4229.db')
cc = lineseq('./dataset/dna_seq_linkers_utf-co.txt')
aa = linkerseq('./dataset/dna_seq_linkers_utf-co.txt', 0) 
